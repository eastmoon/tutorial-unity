...
{
	/// <summary>
	/// It is a simple Tween.
	/// Every Run method will return value of per frame.
	/// </summary>
	/// <example>
	/// <code>
	/// // C#
	/// // Tween( Equation, From, To, Duration )
	/// Tween anim = new Tween(Tween.Equations.Linear, 1, 0, 30);
	/// currentValue = anim.Run(); 
	/// </code>
	/// </example>
	public class Tween
	{
		...
		#region Variable
		private MethodInfo m_easingMethod;
		private Equations m_equation;
		public Equations Equation
		{
			get { return this.m_equation; }
		}
		...
		public Tween( Equations type, double from, double to, double duration )
		{
			this.m_equation = type;
			this.m_easingMethod = this.GetType().GetMethod( type.ToString() );
			this.m_from = from;
			this.m_to = to;
		...
		public double Run()
		{
			try
			{
				...
				{
					object[] methodParams = new object[4] { this.m_currentTime, this.From, this.To - this.From, this.Duration };	
					return ( double ) this.m_easingMethod.Invoke( this, methodParams );
				}
				else
					return this.To;
			}
